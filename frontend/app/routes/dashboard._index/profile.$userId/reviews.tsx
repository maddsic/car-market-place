...
export default function Reviews({ reviews }: ReviewTabProps) {
  const totalReviews = reviews && reviews.length > 0 && reviews.length;
  const isReviewAvailable = reviews && reviews.length > 0 && reviews.length;

  return (
    <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      <h2 className="mb-4 text-2xl font-bold text-gray-800">
        Reviews <span className="text-base text-gray-500">{totalReviews}</span>
      </h2>
      <hr className="mb-6" />
